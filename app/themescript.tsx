...
export default function ThemeScript() {
  useEffect(() => {
    ...
    (async () => {
      // Import the Material Color Utilities
      ...
      function setTheme(sourceColor: string, variant = 'dark', suffix = '') {
        ...
        const scheme = new MCU.DynamicScheme({
          ...
          primaryPalette: MCU.TonalPalette.fromHueAndChroma(hct.hue, hct.chroma * 0.8),
          secondaryPalette: MCU.TonalPalette.fromHueAndChroma(hct.hue, hct.chroma * 0.4),
          tertiaryPalette: MCU.TonalPalette.fromHueAndChroma(hct.hue + 60.0, hct.chroma * 0.6),

          neutralPalette: MCU.TonalPalette.fromHueAndChroma(hct.hue, Math.min(hct.chroma / 10, 8)),
          neutralVariantPalette: MCU.TonalPalette.fromHueAndChroma(hct.hue, Math.min(hct.chroma / 5, 16)),
        });
        ...
        tokens.forEach((token) => {
          ...
          if ((MCU.MaterialDynamicColors as any)[camelToken]) {
            const argb = (MCU.MaterialDynamicColors as any)[camelToken].getArgb(scheme);
            target.style.setProperty(`--md-sys-color-${token}${suffix}`, rgbStr(argb));
          }
        });
      }
      ...
      intervalId = setInterval(() => {
        const dsmColor = (window as any).DSM?.pluginSettings?.['set-primary-color']?.primaryColor;
